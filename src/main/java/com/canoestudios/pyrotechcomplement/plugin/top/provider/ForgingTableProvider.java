...
public class ForgingTableProvider
  ...
  public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, IBlockState blockState, IProbeHitData data) {
    ...
    TileForgingTable tile = (TileForgingTable) tileEntity;
    ...
    ForgingTableRecipe recipe = tile.getRecipe();
    ...
    ItemStack secondaryInput = tile.getSecondaryInput();
    if (!secondaryInput.isEmpty()) {
      horizontal.item(secondaryInput);
    }

    if (recipe != null) {
      horizontal.progress(tile.getProgress(), recipe.getHits(), new ProgressStyle().height(18).width(64).showText(false));
